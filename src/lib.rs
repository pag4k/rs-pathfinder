...
impl<T: Clone+Copy+Eq+Hash+Walkable+AStarHeuristic> Pathfinder<T> {
    ...
    pub fn from_2dvec(map:&[Vec<T>]) -> Pathfinder<T> {
        ...
        vertex_map.iter().for_each(|(vertex, &index)| {
            ...
            .for_each(|element| { graph.insert_edge(1, index, vertex_map[element]).unwrap(); })
        });
    ...
    pub fn get_neighbors(&self, element:&T) -> Option<Vec<T>> {
        ...
        .map(|&edge_index| self.graph.get_vertex_element(self.graph.opposite(vertex_index, edge_index).unwrap()).unwrap())
        .cloned()
        .collect())
    }
    ...
    pub fn dijkstra(&self, start_element: &T, end_element: &T) -> Option<Vec<T>> {
        ...
        if start_index.is_none() || end_index.is_none() { return None; }
        ...
        binary_heap.push( VertexDistance { index: start_index, distance: 0});

        //Instantiate HashMap to trace back path.
        let mut came_from: HashMap<usize, usize> = HashMap::new();

        //Set all distance to maximum usize value.
        let mut distance: HashMap<usize, usize> =  graph.vertices().iter().map(|vertex| (vertex.index, usize::MAX)).collect();
        ...
        'outer: while !binary_heap.is_empty() {
            ...
            if current_distance <= distance.get(&current_index).cloned().unwrap() {
                ...
                if graph.outgoing_edges(current_index).is_none() { continue; }
                //For each outgoing edge, update came_from and distance if shorter path.
                'inner: for edge_index in graph.outgoing_edges(current_index).unwrap().iter().cloned() {
                    //Get opposite node.
                    let opposite_index: usize = graph.opposite(current_index, edge_index).unwrap();
                    //Get potential new distance.
                    let new_distance: usize = distance.get(&current_index).unwrap() + graph.get_edge_element(edge_index).unwrap();
                    if new_distance < distance.get(&opposite_index).unwrap().clone() {
                        binary_heap.push(VertexDistance { index: opposite_index, distance: new_distance});
                        distance.insert(opposite_index, new_distance);
                        came_from.insert(opposite_index, current_index);
                        //If done with end node, break.
                        if opposite_index == end_index { break 'outer; }
                    }
                }
            }
        }
        //If no path was found, abort.
        if !came_from.contains_key(&end_index) { return None; }

        //Build shortest path using came_from.
        let mut path_indices = vec!(end_index);
        loop {
            let previous_index: usize = path_indices.last().unwrap().clone();
            if previous_index == start_index { break; }
            path_indices.push(came_from.get(&previous_index).unwrap().clone());
        }
    ...
    pub fn astar_path(&self, start_element: &T, end_element: &T) -> Option<Vec<T>> {
        ...
        loop {
            ...
            .filter(|neighbor_index| !closed_set.contains(&self.graph.get_vertex_element(*neighbor_index).unwrap()))
...
mod tests {
    ...
    fn test_dijsktra() {
        ...
        assert!(pathfinder.dijkstra(&map[1][1], &map[3][3]).unwrap().pop().unwrap() == map[3][3]);
        assert!(pathfinder.dijkstra(&map[1][1], &map[3][3]).unwrap().pop().unwrap() != map[4][3]);
